...
class ChatbotEngine {
    ...
    // ==========================================
    async processMessage(conversationId, organizationId, messageText, senderPhone, isNewConversation) {
        try {
            ...
            const chatbot = await chatbot_service_1.chatbotService.findMatchingChatbot(organizationId, messageText, isNewConversation && !session);
            if (!chatbot) {
            ...
            // Process flow
            const flowResponses = await this.processFlow(flowData, session, messageText, chatbot);
    ...
    // PROCESS FLOW
    // ==========================================
    async processFlow(flowData, session, userMessage, chatbot) {
        const responses = [];
            ...
            switch (currentNode.type) {
                case 'message':
                    ...
                    if (currentNode.data.message) {
                        const processedMessage = this.processVariables(currentNode.data.message, session.variables);
                ...
                case 'condition':
                    ...
                    const conditionResult = this.evaluateCondition(currentNode.data.condition, userMessage, session.variables);
                    // Find the correct edge based on condition
                    const conditionEdge = edges.find(e => e.source === currentNode.id &&
    ...
    // GET NEXT NODE
    // ==========================================
    getNextNode(currentNodeId, edges, nodes) {
    ...
    // EVALUATE CONDITION
    // ==========================================
    evaluateCondition(condition, userMessage, variables) {
    ...
    // EXECUTE ACTION
    // ==========================================
    async executeAction(node, session) {
    ...
    // SEND RESPONSES VIA WHATSAPP
    // ==========================================
    async sendResponses(organizationId, recipientPhone, responses) {
        try {
            ...
            for (let i = 0; i < responses.length; i++) {
                ...
                await whatsapp_service_1.whatsappService.sendTextMessage(organizationId, waAccount.id, recipientPhone, response);
    ...
    // ==========================================
    async handleButtonResponse(conversationId, buttonPayload) {

...
class BillingService {
    async getCurrentPlan(organizationId) {
        ...
        if (!plan) {
            const freePlan = await database_1.default.plan.findUnique({ where: { type: 'FREE' } });
            if (!freePlan)
                throw new errorHandler_1.AppError('Free plan not found', 500);
            return {
                plan: {
                    id: freePlan.id,
                    name: freePlan.name,
    ...
    async getUsageStats(organizationId) {
        ...
        const plan = organization.subscription?.plan;
        const subscription = organization.subscription;
        const isFreeDemo = (organization.planType === client_1.PlanType.FREE);
        // messages count this month
        const startOfMonth = new Date();
        startOfMonth.setDate(1);
        startOfMonth.setHours(0, 0, 0, 0);
        const messagesThisMonth = await database_1.default.message.count({
            where: {
    ...
    async getAvailablePlans(organizationId) {
        const [plans, organization] = await Promise.all([
            database_1.default.plan.findMany({ where: { isActive: true }, orderBy: { monthlyPrice: 'asc' } }),
            database_1.default.organization.findUnique({ where: { id: organizationId } }),
        ]);
    ...
    async upgradePlan(organizationId, userId, input) {
        ...
        if (organization.ownerId !== userId)
            throw new errorHandler_1.AppError('Only owner can upgrade plan', 403);
        const newPlan = await database_1.default.plan.findUnique({ where: { type: planType } });
        if (!newPlan)
        ...
        const subscription = await database_1.default.$transaction(async (tx) => {
    ...
    async cancelSubscription(organizationId, userId) {
        ...
        if (!organization)
            throw new errorHandler_1.AppError('Organization not found', 404);
        if (organization.ownerId !== userId)
            throw new errorHandler_1.AppError('Only owner can cancel subscription', 403);
        if (!organization.subscription)
            throw new errorHandler_1.AppError('No active subscription', 400);
        if (organization.planType === 'FREE')
            throw new errorHandler_1.AppError('Cannot cancel free plan', 400);
    ...
    async getPaymentMethods(_organizationId) {
        return [
            {
                id: 'pm_1',
                type: 'card',
    ...
    async addPaymentMethod(_organizationId, input) {
        return {
            id: `pm_${Date.now()}`,
            type: input.type,
    ...
    async deletePaymentMethod(_organizationId, _paymentMethodId) {
        return { message: 'Payment method removed successfully' };
    }
    async setDefaultPaymentMethod(_organizationId, _paymentMethodId) {

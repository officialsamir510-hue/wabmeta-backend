...
class MessageQueueWorker extends EventEmitter {
    ...
    private async processNextBatch(workerId: number): Promise<void> {
        const startTime = Date.now();

        const messages = await (prisma as any).messageQueue.findMany({
            where: {
                OR: [
        ...
        for (const message of messages) {
            ...
            try {
                await (prisma as any).messageQueue.update({
                    where: { id: message.id },
    ...
    private async processMessage(queueItem: any): Promise<ProcessResult> {
        try {
            ...
            const accountWithToken = await metaService.getAccountWithToken(whatsappAccount.id);
    ...
    private async checkRateLimits(account: any): Promise<boolean> {
    ...
    private async createConversationMessage(queueItem: any, waMessageId: string) {
        try {
            const { contact, whatsappAccount, template, templateParams } = queueItem;
    ...
    private parseTemplateParams(params: any): any {
    ...
    async addToQueue(data: {
        ...
        const queueItem = await (prisma as any).messageQueue.create({
            data: {
    ...
    async addBatchToQueue(messages: Array<{
        campaignId?: string;
        contactId: string;
        whatsappAccountId: string;
        templateId: string;
        ...
        const result = await (prisma as any).messageQueue.createMany({
            data: queueItems,
        });
    ...
    async getQueueStats() {
        const [pending, processing, sent, failed, total] = await Promise.all([
            (prisma as any).messageQueue.count({ where: { status: QueueStatus.PENDING } }),
            (prisma as any).messageQueue.count({ where: { status: QueueStatus.PROCESSING } }),
            (prisma as any).messageQueue.count({ where: { status: QueueStatus.SENT } }),
            (prisma as any).messageQueue.count({ where: { status: QueueStatus.FAILED } }),
            (prisma as any).messageQueue.count(),
        ]);
    ...
    async cleanupOldMessages(daysOld: number = 30) {
        ...
        const result = await (prisma as any).messageQueue.deleteMany({
            where: {
                OR: [
                    { status: QueueStatus.SENT },
                    { status: QueueStatus.FAILED },
    ...
    async cancelPendingMessages(campaignId: string) {
        const result = await (prisma as any).messageQueue.updateMany({
            where: {
                campaignId,
